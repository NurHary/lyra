...
// that warning
static inline void
Clay__SuppressUnusedLatchDefinitionVariableWarning(void) {
...
typedef CLAY_PACKED_ENUM{
    ...
    CLAY_ERROR_TYPE_HASH_MAP_CAPACITY_EXCEEDED} Clay_ErrorType;
...
typedef struct Clay_ErrorData {
  ...
  // this so we can fix it! CLAY_ERROR_TYPE_UNBALANCED_OPEN_CLOSE -
  // Clay__OpenElement was called more times than Clay__CloseElement, so there
  // were still remaining open elements when the layout ended.
  // CLAY_ERROR_TYPE_HASH_MAP_CAPACITY_EXCEEDED - Clay ran out of capacity in
  // its internal hash map for storing element IDs -> elements. This limit can
  // be increased with Clay_SetMaxElementCount().
...
CLAY_DLL_EXPORT uint32_t
Clay_MinMemorySize(void);
...
Clay_SetPointerState(Clay_Vector2 position, bool pointerDown);
...
CLAY_DLL_EXPORT void
Clay_UpdateScrollContainers(bool enableDragScrolling, Clay_Vector2 scrollDelta,
                            float deltaTime);
...
CLAY_DLL_EXPORT Clay_Vector2
Clay_GetScrollOffset(void);
// Updates the layout dimensions in response to the window or outer container
// being resized.
CLAY_DLL_EXPORT void
Clay_SetLayoutDimensions(Clay_Dimensions dimensions);
// Returns the current dimensions set by Clay_SetLayoutDimensions.
CLAY_DLL_EXPORT Clay_Dimensions
Clay_GetLayoutDimensions(void);
// Called before starting any layout declarations.
CLAY_DLL_EXPORT void
Clay_BeginLayout(void);
...
// can't be used.
CLAY_DLL_EXPORT Clay_ElementId
Clay_GetElementIdWithIndex(Clay_String idString, uint32_t index);
...
// for dynamic strings.
CLAY_DLL_EXPORT Clay_ElementData
Clay_GetElementData(Clay_ElementId id);
...
CLAY_DLL_EXPORT void
Clay_OnHover(void (*onHoverFunction)(Clay_ElementId elementId,
...
CLAY_DLL_EXPORT bool
Clay_PointerOver(Clay_ElementId elementId);
// Returns the array of element IDs that the pointer is currently over.
CLAY_DLL_EXPORT Clay_ElementIdArray
Clay_GetPointerOverIds(void);
...
CLAY_DLL_EXPORT void
Clay_SetMeasureTextFunction(Clay_Dimensions (*measureTextFunction)(
...
CLAY_DLL_EXPORT void
Clay_SetQueryScrollOffsetFunction(Clay_Vector2 (*queryScrollOffsetFunction)(
                                      uint32_t elementId, void* userData),
                                  void* userData);
...
CLAY_DLL_EXPORT void
Clay_SetDebugModeEnabled(bool enabled);
// Returns true if Clay's internal debug tools are currently enabled.
CLAY_DLL_EXPORT bool
Clay_IsDebugModeEnabled(void);
...
// configuration.
CLAY_DLL_EXPORT int32_t
Clay_GetMaxElementCount(void);
...
CLAY_DLL_EXPORT void
Clay_SetMaxElementCount(int32_t maxElementCount);
...
CLAY_DLL_EXPORT int32_t
Clay_GetMaxMeasureTextCacheWordCount(void);
...
CLAY_DLL_EXPORT void
Clay_ResetMeasureTextCache(void);
// A built in transition function that uses the "Ease Out" curve
CLAY_DLL_EXPORT bool
Clay_EaseOut(Clay_TransitionCallbackArguments arguments);

// Internal API functions required by macros ----------------------

CLAY_DLL_EXPORT void
Clay__OpenElement(void);
CLAY_DLL_EXPORT void
Clay__OpenElementWithId(Clay_ElementId elementId);
CLAY_DLL_EXPORT void
...
CLAY_DLL_EXPORT Clay_ElementId
Clay__HashString(Clay_String key, uint32_t seed);
CLAY_DLL_EXPORT Clay_ElementId
Clay__HashStringWithOffset(Clay_String key, uint32_t offset, uint32_t seed);
CLAY_DLL_EXPORT void
Clay__OpenTextElement(Clay_String text, Clay_TextElementConfig textConfig);
...
void
Clay__ErrorHandlerFunctionDefault(Clay_ErrorData errorText) {
  (void)errorText;
}
...
Clay__WarningArray
Clay__WarningArray_Allocate_Arena(int32_t capacity, Clay_Arena* arena);
Clay__Warning*
Clay__WarningArray_Add(Clay__WarningArray* array, Clay__Warning item);
void*
Clay__Array_Allocate_Arena(int32_t capacity, uint32_t itemSize,
                           Clay_Arena* arena);
bool
Clay__Array_RangeCheck(int32_t index, int32_t length);
bool
Clay__Array_AddCapacityCheck(int32_t length, int32_t capacity);
...
Clay_Context*
Clay__Context_Allocate_Arena(Clay_Arena* arena) {
...
Clay_String
Clay__WriteStringToCharBuffer(Clay__charArray* buffer, Clay_String string) {
...
Clay_LayoutElement*
Clay__GetOpenLayoutElement(void) {
...
uint32_t
Clay__GetParentElementId(void) {
  return Clay__GetParentElement()->id;
}

bool
Clay__BorderHasAnyWidth(Clay_BorderElementConfig* borderConfig) {
...
Clay_ElementId
Clay__HashNumber(const uint32_t offset, const uint32_t seed) {
  uint32_t hash = seed;
  hash += (offset + 48);
  hash += (hash << 10);
  hash ^= (hash >> 6);
...
Clay_ElementId
Clay__HashString(Clay_String key, const uint32_t seed) {
  uint32_t hash = seed;
...
Clay_ElementId
Clay__HashStringWithOffset(Clay_String key, const uint32_t offset,
                           const uint32_t seed) {
  uint32_t hash = 0;
  uint32_t base = seed;
...
Clay__SIMDRotateLeft(__m128i x, int r) {
  return _mm_or_si128(_mm_slli_epi64(x, r), _mm_srli_epi64(x, 64 - r));
}

static inline void
Clay__SIMDARXMix(__m128i* a, __m128i* b) {
  *a = _mm_add_epi64(*a, *b);
  *b = _mm_xor_si128(Clay__SIMDRotateLeft(*b, 17), *a);
}

uint64_t
Clay__HashData(const uint8_t* data, size_t length) {
...
#elif !defined(CLAY_DISABLE_SIMD) && defined(__aarch64__)
static inline void
Clay__SIMDARXMix(uint64x2_t* a, uint64x2_t* b) {
  *a = vaddq_u64(*a, *b);
...
uint32_t
Clay__HashStringContentsWithConfig(Clay_String* text,
                                   Clay_TextElementConfig* config) {
  uint32_t hash = 0;
...
Clay__MeasuredWord*
Clay__AddMeasuredWord(Clay__MeasuredWord word,
...
bool
Clay__PointIsInsideRect(Clay_Vector2 point, Clay_BoundingBox rect) {
...
Clay__AddHashMapItem(Clay_ElementId elementId,
  ...
  if (context->layoutElementsHashMapInternal.length ==
    ...
    if (!context->booleanWarnings.hashMapCapacityExceeded) {
      context->errorHandler.errorHandlerFunction(CLAY__INIT(Clay_ErrorData){
          ...
          .errorText = CLAY_STRING(
              "Clay has run out of space in it's internal element ID hashmap.  "
              "Try using Clay_SetMaxElementCount() with a higher value."),
          .userData = context->errorHandler.userData});
  ...
  if (context->layoutElementsHashMapFreeList.length > 0) {
    indexToUse = Clay__int32_tArray_GetValue(
        &context->layoutElementsHashMapFreeList,
        context->layoutElementsHashMapFreeList.length - 1);
    context->layoutElementsHashMapFreeList.length--;
  } else {
    indexToUse = context->layoutElementsHashMapInternal.length;
  }
  Clay_LayoutElementHashMapItem* hashItem =
      Clay__LayoutElementHashMapItemArray_Set(
          &context->layoutElementsHashMapInternal, indexToUse, item);
  if (hashItemPrevious != -1) {
    Clay__LayoutElementHashMapItemArray_Get(
        &context->layoutElementsHashMapInternal, hashItemPrevious)
        ->nextIndex = (int32_t)indexToUse;
  } else {
    context->layoutElementsHashMap.internalArray[hashBucket] =
        (int32_t)indexToUse;
  }
...
Clay_LayoutElementHashMapItem*
Clay__GetHashMapItem(uint32_t id) {
  Clay_Context* context = Clay_GetCurrentContext();
...
void
Clay__UpdateAspectRatioBox(Clay_LayoutElement* layoutElement) {
...
bool
Clay__MemCmp(const char* s1, const char* s2, int32_t length);
#if !defined(CLAY_DISABLE_SIMD) &&                                             \
    (defined(__x86_64__) || defined(_M_X64) || defined(_M_AMD64))
bool
Clay__MemCmp(const char* s1, const char* s2, int32_t length) {
...
void
Clay__ConfigureOpenElementPtr(const Clay_ElementDeclaration* declaration) {
  Clay_Context* context = Clay_GetCurrentContext();
  Clay_LayoutElement* openLayoutElement = Clay__GetOpenLayoutElement();
...
void
Clay__ConfigureOpenElement(const Clay_ElementDeclaration declaration) {
  Clay__ConfigureOpenElementPtr(&declaration);
}

void
Clay__InitializeEphemeralMemory(Clay_Context* context) {
...
void
Clay__InitializePersistentMemory(Clay_Context* context) {
...
bool
Clay__FloatEqual(float left, float right) {
  float subtracted = left - right;
  return subtracted < CLAY__EPSILON && subtracted > -CLAY__EPSILON;
}

Clay_SizingAxis
Clay__GetElementSizing(Clay_LayoutElement* element, bool xAxis) {
  if (element->isTextElement) {
    return CLAY__INIT(Clay_SizingAxis){};
  } else {
    return xAxis ? element->config.layout.sizing.width
                 : element->config.layout.sizing.height;
  }
}

// Writes out the location of text elements to layout elements buffer 1
void
Clay__SizeContainersAlongAxis(bool xAxis, float deltaTime,
    ...
    for (int32_t i = 0; i < bfsBuffer.length; ++i) {
      ...
      for (int32_t childOffset = 0; childOffset < parent->children.length;
        ...
        Clay_SizingAxis childSizing =
            Clay__GetElementSizing(childElement, xAxis);
        ...
        if (!childElement->isTextElement && aspectRatioElementsOut &&
        ...
        if (sizeToDistribute < 0) {
          ...
          for (int childIndex = 0; childIndex < resizableContainerBuffer.length;
            ...
            Clay__SizingType childSizing =
                Clay__GetElementSizing(child, xAxis).type;
          ...
          while (sizeToDistribute > CLAY__EPSILON &&
            ...
            for (int childIndex = 0;
              ...
              float* childSize =
                  xAxis ? &child->dimensions.width : &child->dimensions.height;
              Clay_SizingAxis childSizing =
                  Clay__GetElementSizing(child, xAxis);
...
Clay_String
Clay__IntToString(int32_t integer) {
  if (integer == 0) {
...
void
Clay__AddRenderCommand(Clay_RenderCommand renderCommand) {
  Clay_Context* context = Clay_GetCurrentContext();
...
bool
Clay__ElementIsOffscreen(Clay_BoundingBox* boundingBox) {
  Clay_Context* context = Clay_GetCurrentContext();
...
void
Clay__CalculateFinalLayout(float deltaTime, bool useStoredBoundingBoxes,
  ...
  for (int32_t rootIndex = 0;
    ...
    while (dfsBuffer.length > 0) {
      ...
      Clay_LayoutConfig* layoutConfig = currentElement->isTextElement
                                            ? &CLAY_LAYOUT_DEFAULT
                                            : &currentElement->config.layout;
      ...
      if (!currentElement->isTextElement) {
        if (useStoredBoundingBoxes &&
            currentElement->config.transition.handler) {
          bool found = false;
          for (int j = 0; j < context->transitionDatas.length; ++j) {
            ...
            if (transitionData->elementId == currentElement->id) {
              ...
              if (transitionData->state != CLAY_TRANSITION_STATE_IDLE) {
                if ((transitionData->activeProperties &
...
void
Clay__RenderElementConfigTypeLabel(Clay_String label, Clay_Color color,
                                   bool offscreen) {
...
void
Clay__RenderDebugLayoutSizing(Clay_SizingAxis sizing,
...
void
Clay__DebugViewRenderElementConfigHeader(Clay_String elementId,
                                         Clay__DebugElementConfigType type) {
  Clay__DebugElementConfigTypeLabelConfig config =
...
void
Clay__RenderDebugViewColor(Clay_Color color,
                           Clay_TextElementConfig textConfig) {
...
void
Clay__RenderDebugViewCornerRadius(Clay_CornerRadius cornerRadius,
                                  Clay_TextElementConfig textConfig) {
  CLAY_AUTO_ID({.layout = {.childAlignment = {.y = CLAY_ALIGN_Y_CENTER}}}) {
...
void
HandleDebugViewCloseButtonInteraction(Clay_ElementId elementId,
                                      Clay_PointerData pointerInfo,
                                      void* userData) {
  Clay_Context* context = Clay_GetCurrentContext();
  (void)elementId;
...
void
Clay_SetPointerState(Clay_Vector2 position, bool isPointerDown) {
  Clay_Context* context = Clay_GetCurrentContext();
  if (context->booleanWarnings.maxElementsExceeded) {
    return;
  }
  ...
  for (int32_t rootIndex = context->layoutElementTreeRoots.length - 1;
    ...
    while (dfsBuffer.length > 0) {
      ...
      // This check skips mouse interactions for elements that are currently
      // "exit transitioning"
      if (mapItem && mapItem->generation > context->generation) {
        // Conditionally skip mouse interactions on non-exit transitions, based
        // on user config
        if (!currentElement->isTextElement &&
            currentElement->config.transition.handler) {
...
Clay_EndLayout(float deltaTime) {
  ...
  for (int i = 0; i < context->transitionDatas.length; ++i) {
    ...
    if (!data->transitionOut &&
        (hashMapItem->generation <= context->generation ||
         !hashMapItem->layoutElement->config.transition.handler)) {
  ...
  Clay__int32_tArray elementIdsToRemoveTransitions =
      context->reusableElementIndexBuffer;
    ...
    if (data->transitionOut) {
      ...
      if (hashMapItem->generation <= context->generation) {
        ...
        if (config->exit.trigger ==
          ...
          // This if only runs one single time when the element first starts
          // exiting
          if (data->state != CLAY_TRANSITION_STATE_EXITING) {
            if (parentHashMapItem->generation <= context->generation) {
              ...
              data->elementThisFrame->config.floating.parentId =
                  Clay__HashString(CLAY_STRING("Clay__RootContainer"), 0).id;
            }
            hashMapItem->appearedThisFrame = false;
          ...
          while (bufferIndex < bfsBuffer.length) {
            Clay_LayoutElement* layoutElement =
                Clay_LayoutElementArray_GetCheckCapacity(
                    &context->layoutElements,
                    Clay__int32_tArray_GetValue(&bfsBuffer, bufferIndex));
            Clay_LayoutElementHashMapItem* bfsMapItem =
                Clay__GetHashMapItem(layoutElement->id);
            // Children of exiting elements may have been moved elsewhere in the
            // layout, this prevents a duplicate ID error if they still exist.
            if (bfsMapItem->generation <= context->generation) {
              Clay__AddHashMapItem(
                  CLAY__INIT(Clay_ElementId){layoutElement->id}, layoutElement);
              ...
              for (int j = 0; j < layoutElement->children.length; ++j) {
                ...
                if (childMapItem->generation <= context->generation) {
                  ...
                  if (!childElement->isTextElement &&
                      childElement->config.transition.handler) {
                    Clay__int32_tArray_Add(&elementIdsToRemoveTransitions,
                                           childElement->id);
                  }
                  int32_t childElementIndex =
                      childElement - context->layoutElements.internalArray;
                  Clay_LayoutElement* newChildElement =
                      Clay_LayoutElementArray_Add(&context->layoutElements,
                                                  *childElement);
                  Clay__StringArray_Add(
                      &context->layoutElementIdStrings,
                      *Clay__StringArray_GetCheckCapacity(
              ...
              layoutElement->children = CLAY__INIT(Clay__LayoutElementChildren){
                  .elements = &context->layoutElementChildren
                                   .internalArray[firstChildSlot],
          ...
          // exists and the exiting element is not floating
          Clay_FloatingElementConfig* floatingConfig =
              &hashMapItem->layoutElement->config.floating;
          if (parentHashMapItem->generation > context->generation &&
              floatingConfig->attachTo == CLAY_ATTACH_TO_NONE) {
            ...
            // Otherwise, create the tree root for the floating element (needs
            // to be created every frame)
          } else {
            Clay__LayoutElementTreeRootArray_Add(
                &context->layoutElementTreeRoots,
                CLAY__INIT(Clay__LayoutElementTreeRoot){
  ...
  for (int i = 0; i < elementIdsToRemoveTransitions.length; ++i) {
    for (int j = 0; j < context->transitionDatas.length; ++j) {
      if (Clay__TransitionDataInternalArray_Get(&context->transitionDatas, j)
              ->elementId ==
          Clay__int32_tArray_GetValue(&elementIdsToRemoveTransitions, i)) {
        Clay__TransitionDataInternalArray_RemoveSwapback(
            &context->transitionDatas, j);
        break;
      }
    }
  }
      ...
      for (int i = 0; i < context->transitionDatas.length; ++i) {
        ...
        if (mapItem->appearedThisFrame) {
          ...
          if (transitionData->state != CLAY_TRANSITION_STATE_EXITING) {
            ...
            if (properties & CLAY_TRANSITION_PROPERTY_X) {
              ...
              if (
                  ...
                  !Clay__FloatEqual(oldTargetState.boundingBox.x,
                                    targetState.boundingBox.x)
                  // The element is still in the same parent container, and it's
                  // parent-relative position didn't change (parent moved)
                  && (!(Clay__FloatEqual(oldRelativePosition.x,
                                         newRelativePosition.x)) ||
                      transitionData->reparented)
                  // The position changed was triggered by the outer window
                  // resizing
                  && !context->rootResizedLastFrame) {
            ...
            if (newActiveProperties != 0) {
              ...
              transitionData->activeProperties =
                  (Clay_TransitionProperty)(transitionData->activeProperties |
                                            newActiveProperties);
            }
          }
          ...
          if (transitionData->state == CLAY_TRANSITION_STATE_IDLE) {
            ...
            Clay_ApplyTransitionedPropertiesToElement(
                currentElement, transitionData->activeProperties,
  ...
  for (int i = 0; i < context->layoutElementsHashMap.capacity; ++i) {
    int32_t currentElementIndex =
        context->layoutElementsHashMap.internalArray[i];
    int32_t previousElementIndex = -1;
    while (currentElementIndex != -1) {
      Clay_LayoutElementHashMapItem* currentItem =
          Clay__LayoutElementHashMapItemArray_Get(
              &context->layoutElementsHashMapInternal, currentElementIndex);
      ...
      if (currentItem->generation <= context->generation) {
        ...
        Clay__LayoutElementHashMapItemArray_Set(
            &context->layoutElementsHashMapInternal, currentElementIndex,
            CLAY__INIT(Clay_LayoutElementHashMapItem){.nextIndex = -1});
        Clay__int32_tArray_Add(&context->layoutElementsHashMapFreeList,
                               currentElementIndex);
        ...
        if (previousElementIndex == -1) {
          ...
          Clay_LayoutElementHashMapItem* previousItem =
              Clay__LayoutElementHashMapItemArray_Get(
                  &context->layoutElementsHashMapInternal,
...
bool
Clay_PointerOver(Clay_ElementId elementId) { // TODO return priority for
...
void
Clay_SetExternalScrollHandlingEnabled(bool enabled) {
  Clay_Context* context = Clay_GetCurrentContext();
